...
if __name__ == '__main__':
    lfw_img_dir = osp.join(cfg.ROOT_DIR, 'data', 'lfw', 'dlib-affine-sz_299')

    #caffemodel = os.path.join(cfg.ROOT_DIR, 'output', 'triplet44', 'facenet_triplet_iter_40000.caffemodel')
    caffemodel = os.path.join(cfg.ROOT_DIR, 'output', 'vgg', 'Inception', 'wdecay_iter_40000.caffemodel')
    prototxt = os.path.join(cfg.ROOT_DIR, 'model', 'Inception-Resnet2_deploy.prototxt')
    ...
    mean_arr = np.load(osp.join(cfg.ROOT_DIR, 'data', 'vggface', 'vgg_mean_299.npy'))
    ...
    print num_img
    imdb = np.ndarray((num_img, 2608), dtype=np.float32)
    name_list = []
    ...
    with open('data/lfw_list.txt', 'r') as f:
        for line in f.readlines():
            ...
            blob = np.zeros((1, 299, 299, 3), dtype=np.float32)
            blob[0, :, :, :] = img_resize
    ...
    with open('evaluation/vgg/cls/wd_cls_40000/lfw_embedding.pkl', 'wb') as fid:
